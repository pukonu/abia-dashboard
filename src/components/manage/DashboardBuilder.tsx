...
interface EditorState {
  ...
  title: string;
  span: number;
  /** Stat tiles: show ▲/▼/steady. Default false = simple view. */
  show_change: boolean;
...
function WidgetEditorModal({
  ...
  async function save() {
    ...
    const res = await saveWidgetInline({
      ...
      span: draft.span,
      show_change: draft.chart_type === "stat" && draft.show_change,
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {draft.chart_type === "stat" && (
          <div>
            <div className="mb-1 text-xs font-semibold text-zinc-700">Stat display</div>
            <div className="inline-flex rounded-md border border-zinc-200 p-0.5 dark:border-zinc-700">
              {[
                { value: false, label: "Simple", hint: "Value only" },
                { value: true, label: "With change", hint: "▲ ▼ steady" },
              ].map((o) => (
                <button
                  key={String(o.value)}
                  type="button"
                  onClick={() => setDraft((d) => ({ ...d, show_change: o.value }))}
                  className={`rounded px-3.5 py-1.5 text-left transition-colors ${
                    draft.show_change === o.value
                      ? "bg-zinc-950 text-white dark:bg-zinc-100 dark:text-zinc-950"
                      : "text-zinc-500 hover:text-zinc-800"
                  }`}
                >
                  ...
                  <div
                    className={`text-[10px] leading-snug ${
                      draft.show_change === o.value ? "text-zinc-300 dark:text-zinc-600" : "text-zinc-400"
                    }`}
                  >
                    {o.hint}
                  </div>
            ...
            <p className="mt-1.5 text-[11px] text-zinc-400">
              Simple hides the period-over-period change line. With change shows ▲ up, ▼ down, or steady.
            </p>
          </div>
        )}
        ...
        <div>
          ...
          <div className="max-h-64 overflow-y-auto rounded-md border border-zinc-200">
            ...
            {filtered.map(({ option: o, showGroup }) => (
              <div key={o.id}>
                {showGroup && (
                  <div className="sticky top-0 border-b border-zinc-100 bg-zinc-50 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900">
                    {o.group}
                  </div>
                )}
  ...
  function openEditor(w?: DashboardWidget) {
    setEditor(
      w
        ...
        : { chart_type: "stat", title: "", span: 1, show_change: false, indicator_ids: [] }
    );
  }
    ...
    <div>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          disabled={disabled}
          onClick={() => openEditor()}
          className="inline-flex items-center gap-1.5 rounded-md bg-zinc-950 px-3.5 py-2 text-xs font-semibold text-white transition-colors hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-zinc-100 dark:text-zinc-950 dark:hover:bg-white"
        >
      ...
      {error && (
        <div className="mb-4 rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-900 dark:border-red-900 dark:bg-red-950 dark:text-red-300">
          {error}
        </div>
      )}
