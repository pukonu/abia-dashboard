...
export default async function SectorPage({
  ...
  const thematics = data.thematicAreas.filter((t) => t.sector_id === sector.id);
  ...
  return (
    <>
      ...
      {thematics.map((ta) => {
        ...
        const taDomains = data.domains.filter((d) => d.thematic_area_id === ta.id);
        return (
          <section key={ta.id}>
            <SectionTitle hint={`${FREQ_LABEL[ta.frequency]} reporting`}>
              Thematic area · {ta.name}
            </SectionTitle>
            <div className="card overflow-hidden">
              <div className="flex items-center justify-between gap-3 border-b border-zinc-100 bg-zinc-50/60 px-4 py-3 sm:px-5">
                <p className="text-xs text-zinc-500">{ta.description}</p>
                <ScoreBadge score={taPair.score} showLabel />
              </div>
              {taDomains.map((dom) => {
